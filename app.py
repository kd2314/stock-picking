...
def update_signals():
    ...
    with update_lock:
        ...
        result = get_all_stock_signals()
        global_signals = result['signals']
        last_update_time = datetime.now()
        
        # 保存数据到文件
        with open('stock_signals.json', 'w', encoding='utf-8') as f:
            json.dump({'signals': result['signals'], 'update_time': last_update_time.strftime('%Y-%m-%d %H:%M:%S'), 'end_date': result['end_date']}, f, ensure_ascii=False)
        
        # 保存为CSV文件
        save_signals_to_csv(result['signals'], result['end_date'])
        logger.info("股票信号更新完成")

def save_signals_to_csv(signals, end_date):
    """将信号数据保存为CSV文件"""
    if not signals:
        return
    ...
    csv_filename = os.path.join(history_dir, f'signals_{end_date}.csv')
...
def should_update():
    ...
    if (now.hour > 9 or (now.hour == 9 and now.minute >= 30)) and now.hour < 21:
        return (now - last_update_time).seconds >= 240
    # 非交易时间，每天更新一次
